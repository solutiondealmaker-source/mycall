// Crons — scheduled jobs for iClone.
//
// Schedule summary:
//   every 5 min  — cleanupOrphanBookings (orphan pending → failed)
//   every 5 min  — sendReminders (booking -2h window)
//   every 10 min — detectAbandonedLeads (partialLeads > 10 min without booking)
//   daily 03:00  — autoMarkLostLeads (silence auto-lose)
//   daily 04:00  — truncateNotificationLogs (keep last 200)
...
// Complement to the per-session scheduler arm in capturePartialLead — this
...
// Phase 9 — Renouvelle les watch channels Google Calendar expirant dans les 48h.
// Tourne à 04h00 UTC quotidiennement.
crons.daily(
...
// Phase 9 — Full resync de tous les calendriers Google actifs.
// Tourne à 04h15 UTC (15 min après renew pour que les channels soient frais).
// Corrige la dérive silencieuse due aux push notifications Google manquées.
crons.daily(
